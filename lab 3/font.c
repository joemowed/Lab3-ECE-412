...
/*Font generated by MikroElektronicka GLCD Font Creator*/
...
// GLCD FontSize : 5 x 8
const uint8_t Terminal5x8[] PROGMEM = {
...
/* The font is stored as a bitmap of 8x5 pixels, and the LCD needs bitmaps of
 * 5x8 pixels.  Storing the font as 8x5 bitmaps saves RAM space compared to
 * storing it as 5x8 bitmaps, becuse if stored in 5x8 bitmaps the bits 7:6 would
 * always be zeros and unused.*/
CustomChar decompressChar(const uint8_t *compressedStart) {
    CustomChar ret = {{0, 0, 0, 0, 0, 0, 0, 0}};
    // outer loop: read each byte of the compressed font
    for (uint8_t currentCharacter = 0; currentCharacter < 5;
         currentCharacter++) {
        // inner loop: translate the bits in each compressed font byte to a
        // single bit in each byte of the LCD CustomChar.  This transforms the
        // font from bytes-as-collums to bytes-as-rows.
        for (uint8_t curLine = 0; curLine < 8; curLine++) {
            volatile uint8_t msk =
                (READ_BIT(compressedStart[currentCharacter], curLine));
            msk = (msk > 0);
...
CustomChar getFontChar(uint8_t curChar) {
    ...
    uint16_t index = curChar - 0x20;
    index *= 5;
    // return a LCD CustomChar of the given ascii code generated from the
    // compressed font
	static uint8_t tempArr[5]; 
	for(uint8_t i = 0; i<5;i++){
	tempArr[i] = pgm_read_byte(&Terminal5x8[index+i]);
	}
	CustomChar ret = decompressChar(tempArr);
    return ret;
}
