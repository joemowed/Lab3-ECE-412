...
inline uint16_t microsecondsToPeriodCount(uint16_t value) {
  ...
  return ((value << 4) -
          ((tuningMicroseconds)
           << 4));  // works because 16mhz has 1/16 microsecond period
}
inline void startTimer() {
  DELAY_CTRLB = ((0x1 << WGM12) |
                 (0x1));  // 0x1 sets clk src to cpu clk and starts timer.  CTC
                          // mode auto-resets the count on compareA match
}
...
// mode
// timerinit -inline call to delaymicroseconds takes 2.0us, non-inline with
// return value 10us
